...
Dispatch the `planner` agent (model: opus) with the issue path as input. The agent reads the issue, explores affected surfaces (routes / features / data / generated client / docs), writes the detailed plan into the issue file itself, and updates `docs/issues/status.md` to `planned`.
